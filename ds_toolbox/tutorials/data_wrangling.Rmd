...
```{r}
sampleado <- sample_frac(df_xlsx, 0.1, replace = TRUE)
dim(sampleado)

sampleado <- sample_n(df_xlsx, 15, replace = TRUE)
dim(sampleado)

check_df <- df_xlsx %>% 
filter(SUBTIPO == 'EXTORSION',ENTIDAD =='AGUASCALIENTES') %>%
select(ENTIDAD, SUBTIPO,AÑO,ENERO) 
```


https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf

#Pipeline de Wrangling OPIs (PAsos Data Wrangling)

Suponiendo que los csv están en el hadoop se tiene un proceso de tydification
y una vez en estado tidy (básicamente es reshape de las tablas) se realizan los
siguientes pasos.

00. descarga del hdfs
	-extracción de datos (df)
	-extracción metadatos (de fuente y de vars. mdf,fmdf)

01. limpieza
	- codificación de vacíos
	- hologación claves/nombres
	- codificación columnas

02 agregaciones
	– cálculo agg’s temporales superiores (df’s)
	– cálculo agg’s espaciales superiores (df’s)
	– generación metadata (mdf’s)

03 quintiles
	– calculo quintiles por variable por agg_geo (usa spantemporal)
	– actualización metadf
...
07 transformaciones
	– lectura df’s y mdf’s
	– ejecución transformaciones
	– escritura df’s y mdf’s
